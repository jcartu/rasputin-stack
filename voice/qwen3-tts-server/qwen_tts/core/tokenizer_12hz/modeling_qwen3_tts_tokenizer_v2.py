...
class Qwen3TTSTokenizerV2Decoder(Qwen3TTSTokenizerV2DecoderPreTrainedModel):
    ...
    def chunked_decode(self, codes, chunk_size=300, left_context_size=25):
        wavs = []
        start_index = 0
        pad_target = chunk_size + left_context_size
        while start_index < codes.shape[-1]:
            ...
            if self._compiled_forward is not None:
                wav_chunk = self.decode_padded(codes_chunk, pad_target) + 0
            else:
                wav_chunk = self(codes_chunk) + 0
    ...
    def compile_for_streaming(self, mode: str = "reduce-overhead", backend: str = "inductor"):
        ...
        print("[Decoder] Note: mode='reduce-overhead' includes CUDA graphs automatically")
...
class Qwen3TTSTokenizerV2Model(Qwen3TTSTokenizerV2PreTrainedModel):
    ...
    def enable_streaming_optimizations(
        ...
        """
        print("[Tokenizer] Enabling streaming optimizations...")
        ...
        if use_cuda_graphs:
            if compile_mode == "reduce-overhead":
                print("[Tokenizer] Note: compile_mode='reduce-overhead' already includes CUDA graphs")
                print("[Tokenizer] Manual CUDA graph capture skipped (not needed)")
            else:
