"""Pipecat Voice Agent — Qwen3-TTS + Parakeet STT + llamaswap"""
...
class SanitizedLLMService(OpenAILLMService):
    """Merges consecutive user messages to prevent strict template errors after interruptions."""

    async def _process_context(self, context):
        if hasattr(context, 'messages'):
            i = 1
            while i < len(context.messages):
                if (context.messages[i].get("role") == "user" and
                        context.messages[i - 1].get("role") == "user"):
                    prev = context.messages[i - 1].get("content", "")
                    curr = context.messages[i].get("content", "")
                    context.messages[i - 1]["content"] = f"{prev}\n{curr}".strip()
...
class StreamingOpenAITTSService(OpenAITTSService):
    ...
    def __init__(self, *, chunk_seconds: float = 0.5, **kwargs):
        super().__init__(**kwargs)
        self._chunk_seconds = chunk_seconds

    @property
    def chunk_size(self) -> int:
        # Base class uses 0.5s (24000 bytes at 24kHz) — too much for streaming
...
def _create_tts_service(cfg: dict) -> OpenAITTSService:
    ...
    if streaming:
        chunk_seconds = cfg["tts"].get("chunk_seconds", 0.5)
        logger.info(f"TTS mode: streaming (chunk_seconds={chunk_seconds})")
        return StreamingOpenAITTSService(
    ...
    logger.info("TTS mode: non-streaming")
...
# ---------------------------------------------------------------------------
# Bot entry point — called by Pipecat's built-in runner
# ---------------------------------------------------------------------------
async def bot(args: SmallWebRTCRunnerArguments):
    ...
    # Map config language string to Pipecat Language enum
    _lang_map = {"de": Language.DE, "en": Language.EN, "fr": Language.FR,
                 "es": Language.ES, "it": Language.IT, "pl": Language.PL}
    stt_lang = _lang_map.get(cfg["stt"].get("language", "de"), Language.DE)
