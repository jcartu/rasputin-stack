...
"""
    ...
    # GPU keepalive: periodic matmul to prevent AMD DPM idle downclocking
    keepalive_task = None
    if GPU_KEEPALIVE_INTERVAL > 0:
        try:
            import torch
            if torch.cuda.is_available():
                _ka = torch.randn(512, 512, device="cuda:0")
                async def _gpu_keepalive():
                    while True:
                        await asyncio.sleep(GPU_KEEPALIVE_INTERVAL)
                        torch.matmul(_ka, _ka)
                keepalive_task = asyncio.create_task(_gpu_keepalive())
                logger.info(f"GPU keepalive: matmul every {GPU_KEEPALIVE_INTERVAL}s")
        except Exception as e:
            logger.warning(f"GPU keepalive setup failed: {e}")
