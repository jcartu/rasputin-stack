#!/usr/bin/env python3
"""platform Proxy v4 — MiniMax M2.5 direct + full cost intelligence + Cerebras compaction.

v3 → v4 changes:
- MiniMax M2.5 as Anthropic-compatible upstream (api.minimax.io/anthropic)
- Model-based routing: minimax models → MiniMax API, everything else → Anthropic
- Full cost tracking for MiniMax provider
- /providers updated with MiniMax status
"""
...
if __name__ == "__main__":
    import uvicorn
    print(f"[platform PROXY v4] Starting on {BIND_HOST}:{BIND_PORT}")
    print(f"[platform PROXY v4] Cerebras: {CEREBRAS_MODEL}")
    print(f"[platform PROXY v4] MiniMax: {'configured' if MINIMAX_API_KEY else 'NO KEY'}")
    print(f"[platform PROXY v4] Compaction markers: {len(COMPACTION_MARKERS)}")
    print(f"[platform PROXY v4] Cost log: {COST_LOG}")
