#!/usr/bin/env python3
"""platform Proxy v5 — Safety layer + MiniMax hardening + response sanitization.

v4 → v5 changes:
- Command blocklist: intercept dangerous exec commands before forwarding
- Response sanitization: strip CJK leakage, <minimax:tool_call> XML hallucinations
- API key scrubbing: remove sk-* patterns from responses
- All safety checks applied to MiniMax responses (not Opus — Opus handles its own safety)
"""
...
# --- App ---
app = FastAPI(title="platform Proxy v5")
...
async def proxy_messages(request: Request):
    ...
    # Route MiniMax models to MiniMax API
    if any(model.startswith(m) for m in MINIMAX_MODELS) or "minimax" in model:
        ...
        return await route_to_minimax(request, body_bytes, body_json, is_stream)
...
if __name__ == "__main__":
    import uvicorn
    print(f"[platform PROXY v5] Starting on {BIND_HOST}:{BIND_PORT}")
    print(f"[platform PROXY v5] Cerebras: {CEREBRAS_MODEL}")
    print(f"[platform PROXY v5] MiniMax: {'configured' if MINIMAX_API_KEY else 'NO KEY'}")
    print(f"[platform PROXY v5] Safety: {len(DANGEROUS_COMMANDS)} command patterns blocked")
    print(f"[platform PROXY v5] Protected files: {len(PROTECTED_FILES)}")
    print(f"[platform PROXY v5] Secret patterns: {len(SECRET_PATTERNS)}")
    print(f"[platform PROXY v5] Cost log: {COST_LOG}")
