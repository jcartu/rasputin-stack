#!/usr/bin/env python3
"""platform Proxy v7 — Cerebras GLM-4.7 compaction fixes + OpenCode Zen fallback.

v6 → v7 changes:
- Fixed anthropic_to_openai conversion: tool_use/tool_result blocks preserved as text summaries
- Fixed consecutive same-role message merging (Cerebras rejects these)
- Increased max_completion_tokens to 32768 for GLM-4.7 reasoning headroom
- Added detailed Cerebras error logging (HTTP status + response body)
- Compaction fallback chain: Cerebras → OpenCode Zen (free Opus) → Anthropic OAuth (paid)
- All v6 features preserved (quality gate, safety, sanitization)
"""
...
def anthropic_to_openai(body: dict) -> dict:
    ...
    for msg in body.get("messages", []):
        ...
        if isinstance(content, str):
            ...
            # Extract text from text blocks; also extract tool_use/tool_result as text summaries
            parts = []
            for b in content:
            ...
            text = str(content)
        if text.strip():
            # Merge consecutive same-role messages (Cerebras/OpenAI reject them)
    ...
    return {"model": CEREBRAS_MODEL, "messages": messages, "max_completion_tokens": max_tokens,
...
async def proxy_messages(request: Request):
    ...
    if is_compaction(body_json):
        ...
        # Compaction chain: Cerebras GLM-4.7 → OpenCode Zen (free Opus) → Anthropic OAuth (paid, last resort)
        print(f"[COMPACTION] Routing to Cerebras (stream={is_stream})")
        result, ok = await route_to_cerebras(body_json, is_stream)
        if ok:
            return result
        print("[COMPACTION] Cerebras failed, trying OpenCode Zen fallback")

        # Fallback 1: OpenCode Zen (free Opus 4.6)
        zen_result = await route_to_opencode_zen_compaction(request, body_bytes, body_json, is_stream)
        if zen_result is not None:
            return zen_result
        print("[COMPACTION] OpenCode Zen failed, falling through to Anthropic OAuth (paid)")
...
if __name__ == "__main__":
    import uvicorn
    print(f"[platform PROXY v7] Starting on {BIND_HOST}:{BIND_PORT}")
    print(f"[platform PROXY v7] Cerebras: {CEREBRAS_MODEL}")
    print(f"[platform PROXY v7] OpenCode Zen: {'configured' if OPENCODE_ZEN_KEY else 'NO KEY'}")
    print(f"[platform PROXY v7] Compaction chain: Cerebras → OpenCode Zen → Anthropic OAuth")
    print(f"[platform PROXY v7] MiniMax: {'configured' if MINIMAX_API_KEY else 'NO KEY'}")
    print(f"[platform PROXY v7] Quality gate: threshold={QUALITY_THRESHOLD}, auto-escalation to Opus")
    print(f"[platform PROXY v7] Safety: {len(DANGEROUS_COMMANDS)} cmd patterns, {len(PROTECTED_FILES)} files, {len(SECRET_PATTERNS)} secrets")
    print(f"[platform PROXY v7] Cost log: {COST_LOG}")
