#!/usr/bin/env python3
"""Rasputin Proxy v10 — Qwen tool calling support + session-affinity load balancing.
...
MINIMAX_MODELS = {
    ...
    "minimax-m2",
    # PascalCase variants (OpenClaw 2026.3.2+ native provider sends these)
    "MiniMax-M2.5", "MiniMax-M2.5-highspeed",
    "MiniMax-M2.1", "MiniMax-M2.1-highspeed", "MiniMax-M2.1-Lightning",
}
# Local Qwen 3.5 122B-A10B (llama.cpp, IQ3_XXS) — OpenAI-compatible
LOCAL_QWEN_URL = "http://127.0.0.1:${LOCAL_LLM_PORT:-11435}/v1/chat/completions"
...
LOCAL_CODER_URL = "http://127.0.0.1:${LOCAL_CODER_PORT:-11436}/v1/chat/completions"
...
GEMINI_PROXY_URL = "http://127.0.0.1:${GEMINI_PROXY_PORT:-4000}/v1/messages"
...
BIND_HOST = "127.0.0.1"
BIND_PORT = int(os.environ.get("PROXY_PORT", "8889"))
COST_LOG = os.path.join(os.path.dirname(__file__), "cost.jsonl")
CLAUDE_CREDENTIALS = os.path.expanduser("~/.credentials.json")
...
LB_OAUTH = "oauth"
LB_GEMINI = "gemini"
LB_PROVIDERS = [LB_ZEN, LB_OAUTH]  # priority order (Gemini disabled — 2025-02-25)
...
# Zen re-migration: periodically try to move OAuth-stuck sessions back to Zen
_session_reassign_check = {}  # session_id -> next_check_timestamp
ZEN_REMIGRATE_INTERVAL = 300  # 5 minutes between re-migration attempts
...
def _evict_stale_sessions():
    ...
    for k in stale:
        ...
        _session_reassign_check.pop(k, None)
...
def _get_provider_for_session(session_id: str) -> str:
    ...
    if session_id in _session_affinity:
        ...
        # Zen re-migration: if session is stuck on OAuth, periodically try Zen
        if provider == LB_OAUTH:
            now = time.time()
            next_check = _session_reassign_check.get(session_id, 0)
            if now >= next_check and _is_provider_available(LB_ZEN):
                _session_reassign_check[session_id] = now + ZEN_REMIGRATE_INTERVAL
                _session_affinity[session_id] = LB_ZEN
                _session_affinity_ts[session_id] = now
                print(f"[LB] Session {session_id[:8]}… re-migration attempt: OAuth → Zen")
                return LB_ZEN
...
DANGEROUS_COMMANDS = [
    ...
    (r'\bpm2\s+delete\s+(rasputin-proxy|chrome-agent|rasputin)', "delete critical pm2 app"),
]
...
RESPONSE_MODEL_PREFIX = {
    "claude-opus-4-6": "rasputin-proxy/claude-opus-4-6",
    "claude-sonnet-4-6": "rasputin-proxy/claude-sonnet-4-6",
    "claude-sonnet-4-5": "rasputin-proxy/claude-sonnet-4-5",
    "claude-opus-4.6": "rasputin-proxy/claude-opus-4-6",
    "claude-sonnet-4.6": "rasputin-proxy/claude-sonnet-4-6",
    "claude-sonnet-4.5": "rasputin-proxy/claude-sonnet-4-5",
    "MiniMax-M2.5": "rasputin-proxy/minimax-m2.5",
    "minimax-m2.5": "rasputin-proxy/minimax-m2.5",
    "MiniMax-M2.5-highspeed": "rasputin-proxy/minimax-m2.5-highspeed",
    "minimax-m2.5-highspeed": "rasputin-proxy/minimax-m2.5-highspeed",
    "MiniMax-M2.1": "rasputin-proxy/minimax-m2.5",
    "minimax-m2.1": "rasputin-proxy/minimax-m2.5",
    "MiniMax-M2.1-highspeed": "rasputin-proxy/minimax-m2.5-highspeed",
    "MiniMax-M2.1-Lightning": "rasputin-proxy/minimax-m2.5-highspeed",
    "minimax-m2.1-highspeed": "rasputin-proxy/minimax-m2.5-highspeed",
    "qwen3.5-122b-a10b": "rasputin-proxy/qwen3.5-122b-a10b",
    "qwen-122b": "rasputin-proxy/qwen3.5-122b-a10b",
    "qwen-local": "rasputin-proxy/qwen3.5-122b-a10b",
    "local-qwen": "rasputin-proxy/qwen3.5-122b-a10b",
}
...
# --- App ---
app = FastAPI(title="Rasputin Proxy v7")
...
async def route_to_local_qwen(request: Request, body_bytes: bytes, body_json: dict, is_stream: bool, override_url: str = None):
    ...
    try:
        ...
        # Qwen3.5 optimal sampling (official recommendations — Mar 3 2026)
        # temp 0 causes infinite repetition; 0.6 is sweet spot for tool calling
        openai_body.setdefault("temperature", 0.6)
        openai_body.setdefault("top_k", 20)
        openai_body.setdefault("top_p", 0.95)
        openai_body.setdefault("presence_penalty", 1.5)
        openai_body.setdefault("min_p", 0.0)
        ...
        if is_stream:
            ...
            return StreamingResponse(local_stream_gen(), media_type="text/event-stream",
                                     headers={"X-Rasputin-Provider": provider})
        else:
...
async def proxy_messages(request: Request):
    ...
    # Gemini routing disabled — 2025-02-25
    ...
    if is_opus_request:
        ...
        while provider != "none" and provider not in tried:
            ...
            if provider == LB_ZEN:
                ...
                # Strip known prefixes: opencode/, rasputin-proxy/, zen/
                for pfx in ("opencode/", "rasputin-proxy/", "zen/"):
        ...
        for pfx in ("opencode/", "rasputin-proxy/", "zen/", "anthropic-direct/"):
            if raw_model.startswith(pfx):
                raw_model = raw_model[len(pfx):]
                break
...
async def providers_endpoint():
    ...
    return {"providers": providers, "failover_chain": [
        "rasputin-proxy/claude-opus-4-6 → Anthropic API (OAuth)",
        "opencode/claude-opus-4-6 (Zen, free)",
        "rasputin-proxy/gemini-3.1-pro-preview → Gemini via translation proxy ($2/$12)",
...
if __name__ == "__main__":
    import uvicorn
    print(f"[RASPUTIN PROXY v10] Starting on {BIND_HOST}:{BIND_PORT}")
    print(f"[RASPUTIN PROXY v10] Load balancer: Zen → OAuth → Gemini (session-affinity)")
    print(f"[RASPUTIN PROXY v10] Qwen tool calling: ENABLED (full schema translation)")
    print(f"[RASPUTIN PROXY v10] OpenCode Zen: {'configured' if OPENCODE_ZEN_KEY else 'NO KEY'}")
    print(f"[RASPUTIN PROXY v10] OAuth thresholds: 5h>{OAUTH_5H_SKIP_THRESHOLD*100:.0f}% 7d>{OAUTH_7D_SKIP_THRESHOLD*100:.0f}%")
    print(f"[RASPUTIN PROXY v10] MiniMax: {'configured' if MINIMAX_API_KEY else 'NO KEY'}")
    print(f"[RASPUTIN PROXY v10] Quality gate: best-of-{BEST_OF_N}, threshold={QUALITY_THRESHOLD}")
    print(f"[RASPUTIN PROXY v10] Safety: {len(DANGEROUS_COMMANDS)} cmd patterns, {len(PROTECTED_FILES)} files, {len(SECRET_PATTERNS)} secrets")
    print(f"[RASPUTIN PROXY v10] Cost log: {COST_LOG}")
