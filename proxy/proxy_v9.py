#!/usr/bin/env python3
"""platform Proxy v9 — Session-affinity load balancing across Zen/OAuth/Gemini.

v8 → v9 changes:
- Session-affinity load balancer: Opus requests distributed across Zen, OAuth, Gemini
- Provider priority: Zen (primary, free) → OAuth (secondary, free) → Gemini (overflow, cheap)
- Sessions stick to assigned provider for cache coherence
- OAuth utilization-aware: auto-skip when 5h utilization > 70% or 7d > 60%
- Automatic failover: if assigned provider fails, cascade to next + reassign session
- All v8 features preserved (best-of-5 MiniMax, compaction, safety, sanitization)
"""
...
async def route_to_local_qwen(request: Request, body_bytes: bytes, body_json: dict, is_stream: bool, override_url: str = None):
    ...
    try:
        openai_body = anthropic_to_openai(body_json)
...
if __name__ == "__main__":
    import uvicorn
    print(f"[platform PROXY v9] Starting on {BIND_HOST}:{BIND_PORT}")
    print(f"[platform PROXY v9] Load balancer: Zen → OAuth → Gemini (session-affinity)")
    print(f"[platform PROXY v9] OpenCode Zen: {'configured' if OPENCODE_ZEN_KEY else 'NO KEY'}")
    print(f"[platform PROXY v9] OAuth thresholds: 5h>{OAUTH_5H_SKIP_THRESHOLD*100:.0f}% 7d>{OAUTH_7D_SKIP_THRESHOLD*100:.0f}%")
    print(f"[platform PROXY v9] MiniMax: {'configured' if MINIMAX_API_KEY else 'NO KEY'}")
    print(f"[platform PROXY v9] Quality gate: best-of-{BEST_OF_N}, threshold={QUALITY_THRESHOLD}")
    print(f"[platform PROXY v9] Safety: {len(DANGEROUS_COMMANDS)} cmd patterns, {len(PROTECTED_FILES)} files, {len(SECRET_PATTERNS)} secrets")
    print(f"[platform PROXY v9] Cost log: {COST_LOG}")
