#!/usr/bin/env python3
"""AI Council & Synthesis System v2 — WITH FRACTAL MODE

New in v2:
    --mode fractal: Spawns 4 sub-agents (Research, Counter-argument, Feasibility, Creative)
                    Each gets 200s to research independently, then Opus 4.6 synthesizes.

Usage:
    python3 ai_council_v2.py "question"                          # Auto mode
    python3 ai_council_v2.py --mode synthesis "question"         # Fast synthesis
    python3 ai_council_v2.py --mode council "question"           # Full debate
    python3 ai_council_v2.py --mode fractal "question"           # Fractal sub-agents
    python3 ai_council_v2.py --panel                             # Show models
    python3 ai_council_v2.py --json "question"                   # JSON output
"""
...
_m("grok-reason",   "grok-4-fast-reasoning",          "xai",         Tier.FRONTIER, "Grok 4.2 Reasoning")
...
_m("grok-fast",     "grok-4-fast-non-reasoning",      "xai",         Tier.STRONG,   "Grok 4.2 Fast")
...
async def call_model(client: httpx.AsyncClient, model: Model, messages: list[dict],
                     max_tokens: int = 1024, timeout_override: Optional[int] = None) -> dict:
    start = time.monotonic()
    timeout = timeout_override if timeout_override is not None else model.timeout
    result = {"model": model.name, "key": model.key, "tier": model.tier.value,
              "weight": model.weight, "content": None, "error": None, "latency": 0}
    try:
        if model.provider == "anthropic":
            # Anthropic API: system prompt is a top-level param, not in messages
            system_text = None
            api_messages = []
            for msg in messages:
                if msg["role"] == "system":
            ...
            r = await client.post("https://api.anthropic.com/v1/messages",
                ...
                timeout=timeout)
            r.raise_for_status()
            result["content"] = r.json()["content"][0]["text"]
...
def get_council_system_prompt():
    ...
    system = (
        "You are an expert AI advisor on a council panel. You're providing analysis and recommendations.\n"
        "KEY CONTEXT:\n"
...
async def run_fractal(client: httpx.AsyncClient, query: str) -> dict:
    """
    FRACTAL MODE: Spawn 4 specialized model calls (Research, Counter-argument, Feasibility, Creative)
    Each runs async with 200s timeout. Collect outputs → Opus 4.6 synthesis.
    """
    t0 = time.monotonic()
    
    p(C.BOLD + C.CYAN, "\n🌀 FRACTAL MODE — 4 Specialized Research Perspectives")
    p(C.DIM, f"   Question: {query}")
    p(C.DIM, "   Models: Sonnet 4.5 (each gets 200s max)\n")
    
    emit("phase", phase="spawning", mode="fractal", agents=4, timeout=200)
    
    system_prompt = get_council_system_prompt()
    research_model = MODELS["sonnet"]  # Use Sonnet 4.5 for sub-agents
    
    # Define the 4 specialized perspectives
    agents = [
        {
            "label": "research",
            "color": C.BLUE,
            "prompt": (
                f"You are the RESEARCH specialist on a fractal council.\n\n"
                f"Question: {query}\n\n"
                f"Your task: Conduct DEEP RESEARCH. Consider:\n"
                f"• Technical feasibility and architecture patterns\n"
                f"• Industry best practices and proven approaches\n"
                f"• Real-world examples and case studies\n"
                f"• Data, benchmarks, and empirical evidence\n"
                f"• Academic literature and expert opinions\n\n"
                f"Provide a comprehensive research report. Be thorough and cite specific examples.\n"
                f"Take your time - you have 200s to produce the definitive research analysis."
            )
        },
        {
            ...
            "color": C.RED,
            "prompt": (
                f"You are the COUNTER-ARGUMENT specialist on a fractal council.\n\n"
                f"Question: {query}\n\n"
                f"Your task: Challenge EVERY assumption. Consider:\n"
                f"• What could go catastrophically wrong?\n"
                f"• Hidden costs, technical debt, maintenance burden\n"
                f"• Alternative perspectives and contrarian viewpoints\n"
                f"• Edge cases and failure modes\n"
                f"• Why the 'obvious' solution might be wrong\n\n"
                f"Be the devil's advocate. Your job is to find flaws, not make friends.\n"
                f"Take your time - you have 200s to produce the strongest counter-arguments."
            )
        },
        {
            "label": "feasibility",
            "color": C.YELLOW,
            "prompt": (
                f"You are the FEASIBILITY specialist on a fractal council.\n\n"
                f"Question: {query}\n\n"
                f"Your task: Analyze PRACTICAL IMPLEMENTATION. Consider:\n"
                f"• Resource requirements (compute, storage, bandwidth, cost)\n"
                f"• Timeline and complexity estimates\n"
                f"• Dependencies and prerequisites\n"
                f"• Risk factors and mitigation strategies\n"
                f"• Incremental rollout and testing approaches\n\n"
                f"Be realistic about what's achievable. Provide concrete implementation steps.\n"
                f"Take your time - you have 200s to produce a detailed feasibility assessment."
            )
        },
        {
            "label": "creative",
            "color": C.MAGENTA,
            "prompt": (
                f"You are the CREATIVE specialist on a fractal council.\n\n"
                f"Question: {query}\n\n"
                f"Your task: Think OUTSIDE THE BOX. Consider:\n"
                f"• Unconventional approaches and lateral thinking\n"
                f"• Novel combinations of existing technologies\n"
                f"• Future possibilities and emerging tech\n"
                f"• What if we broke all the rules?\n"
                f"• Inspired analogies from other domains\n\n"
                f"Don't be constrained by 'conventional wisdom'. Propose bold, innovative solutions.\n"
                f"Take your time - you have 200s to produce breakthrough creative ideas."
            )
        }
    ]
    
    # Launch all 4 specialists in parallel
    p(C.YELLOW, "▸ Launching 4 specialist researchers (parallel)...\n")
    agent_start = time.monotonic()
    
    tasks = []
    for agent in agents:
        p(C.DIM, f"   • {agent['label']:20s} starting...")
        task = call_model(
            client, 
            research_model,
            [{"role": "system", "content": system_prompt}, 
            ...
            max_tokens=4096,
            timeout_override=200  # Give each specialist 200s to complete
        )
        tasks.append((agent['label'], agent['color'], task))
    
    # Gather results
    results = await asyncio.gather(*[t[2] for t in tasks], return_exceptions=True)
    agent_time = time.monotonic() - agent_start
    
    p(C.GREEN, f"\n✓ All specialists completed in {agent_time:.1f}s\n")
    
    # Process results
    agent_outputs = {}
    for i, (label, color, _) in enumerate(tasks):
        result = results[i]
        if isinstance(result, Exception):
            p(C.RED, f"   ✗ {label:20s} error: {str(result)[:80]}")
            emit("agent_error", agent=label, error=str(result))
        elif result.get("error"):
            p(C.RED, f"   ✗ {label:20s} {result['error'][:80]}")
            emit("agent_error", agent=label, error=result["error"])
        else:
            agent_outputs[label] = result["content"]
            p(color, f"   ✓ {label:20s} {result['latency']}s, {len(result['content'])} chars")
            emit("agent_done", agent=label, latency=result["latency"], 
                 chars=len(result['content']))
    
    # Check what we got
    if not agent_outputs:
        p(C.RED, "\n✗ No specialist outputs collected")
        return {"verdict": "Fractal mode failed: no outputs collected", 
                "confidence": 0, "mode": "fractal",
                "total_time": round(time.monotonic() - t0, 2)}
    
    p(C.GREEN, f"\n✓ Collected {len(agent_outputs)}/4 specialist perspectives\n")
    
    # Build synthesis prompt for Opus 4.6
    emit("phase", phase="synthesizing", mode="fractal", judge="Claude Opus 4.6")
    p(C.YELLOW, "▸ Supreme Court synthesis (Opus 4.6)...", end="")
    
    synthesis_text = f"ORIGINAL QUESTION:\n{query}\n\n"
    synthesis_text += "=" * 70 + "\n\n"
    
    for label in ["research", "counter-argument", "feasibility", "creative"]:
        if label in agent_outputs:
            synthesis_text += f"### {label.upper().replace('-', ' ')} SPECIALIST:\n{agent_outputs[label]}\n\n"
            synthesis_text += "=" * 70 + "\n\n"
    
    synthesis_prompt = (
        f"{synthesis_text}\n"
        f"You are the SUPREME COURT judge synthesizing the above 4 specialist perspectives.\n\n"
        f"Produce your FINAL VERDICT:\n"
        f"1. **VERDICT**: The definitive answer synthesizing all 4 perspectives\n"
        f"2. **CONFIDENCE**: 0-100 score\n"
        f"3. **CONSENSUS**: How aligned were the specialists? (unanimous/strong/weak/split)\n"
        f"4. **KEY INSIGHTS**: What each specialist contributed\n"
        f"5. **SYNTHESIS**: How you integrated research, counter-arguments, feasibility, and creativity\n"
        f"6. **RECOMMENDATION**: Your final authoritative recommendation\n\n"
        f"Be comprehensive. This is the definitive answer."
    )
    
    s_start = time.monotonic()
    
    # Call Opus 4.6 for synthesis (with extended timeout for large input)
    synth = await call_model(
        ...
        max_tokens=4096,
        timeout_override=200  # Give Opus 200s for synthesis of all 4 specialists
    )
    s_time = time.monotonic() - s_start
    
    if synth.get("error"):
        p(C.RED, f" ✗ {synth['error'][:100]}")
        verdict = f"Synthesis error: {synth['error']}"
    else:
        p(C.GREEN, f" done ({s_time:.1f}s)")
        verdict = synth.get("content") or "Synthesis failed"
    ...
    p(C.BOLD + C.WHITE, f"\n{'═' * 70}")
    p(C.BOLD + C.CYAN, f" 🌀 FRACTAL VERDICT  │  {total:.1f}s  │  {len(agent_outputs)}/4 specialists  │  Supreme Court")
    p(C.BOLD + C.WHITE, f"{'═' * 70}\n")
    print(verdict)
    p(C.BOLD + C.WHITE, f"\n{'═' * 70}")
    ...
    result = {
        "verdict": verdict,
        "confidence": _extract_int(verdict, "CONFIDENCE", 50),
        "consensus": _extract_word(verdict),
        "specialists": agent_outputs,
        "synthesis": synth,
        "research_time": round(agent_time, 2),
        "total_time": round(total, 2),
        "mode": "fractal"
    }
    
    emit("verdict", verdict=verdict, confidence=result["confidence"], 
         consensus=result["consensus"], total_time=result["total_time"], mode="fractal")
    emit("done", result=result)
...
def save_result(result: dict):
    ...
    path = out_dir / f"council_{mode}_{ts}.json"
    path.write_text(json.dumps(result, indent=2, default=str))
    p(C.DIM, f"\n💾 Saved: {path}")

def show_panel():
    p(C.BOLD + C.WHITE, f"\n{'═' * 70}")
    p(C.BOLD + C.CYAN, " 📋 AI COUNCIL v2 — MODEL REGISTRY")
    p(C.BOLD + C.WHITE, f"{'═' * 70}\n")
    ...
    p(C.CYAN, "    Fractal:   4 sub-agents → Opus 4.6")
...
async def main():
    parser = argparse.ArgumentParser(description="AI Council v2 — Multi-model consensus engine with FRACTAL mode")
    parser.add_argument("query", nargs="?", help="Question to ask")
    parser.add_argument("--mode", choices=["synthesis", "council", "fractal", "auto"], default="auto")
    parser.add_argument("--panel", action="store_true", help="Show available models")
    parser.add_argument("--json", action="store_true", help="JSON output")
    parser.add_argument("--models", help="Comma-separated model keys to use (synthesis/council only)")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if mode == "fractal":
            result = await run_fractal(client, args.query)
        elif mode == "synthesis":
